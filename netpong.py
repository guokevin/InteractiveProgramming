...
class EscapeTheMazeClientModel(object):
    ...
    def update_monster(self):
        ...
        if self.players[self.player_num].rect.colliderect(self.monster.rect) and self.players[self.player_num].color != 'black':
            self.still_alive[self.player_num] = False
...
class PyGameKeyboardController(object):
    ...
    def handle_event(self, event):
        if not self.model.still_alive[self.model.player_num]:
            return
        y_vel = 0
        x_vel = 0
...
class Listener(ConnectionListener): 
    ...
    def Network_is_monster(self, data):
        for i in range (len(self.model.players)):
            if data['monster'][i]:
                self.model.players[i].width = self.model.players[i].width*1.5
                self.model.players[i].height = self.model.players[i].height*1.5
                self.model.players[i].color = "black"
                self.model.players[i].rect = pygame.Rect(self.model.players[i].x_pos,
        ...
        if self.running:
            ...
            if self.start:
                if not self.ran_initiations:
                    print 'before lists'
                    self.model.monster = self.model.players[self.model.monster_num-1]
